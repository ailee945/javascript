...
专门用来表示一个空的对象，`typeof`返回`object`

### Undefined 未定义

未定义，声明未赋值，`typeof`返回`object`
...
- `typeof`返回的是一个`Object`
- `var arr = new Array(1, 2, 3)`使用构造函数创建数组的同时也可以指定数组中的元素
- 读取不存在的索引返回的是`undefined`
- 获取或是**设置**`length`获取数组的长度，`arr.length`
- `arr[arr.length] = 1`向数组的最后一位添加元素
- 使用字面量创建数组`var arr = [1, 2, 3]`
- `push()`方法:向数组末尾添加一个或是多个元素，该方法会将数组的最新长度作为返回值返回
- `pop()`方法：删除数组的最后一个元素，该方法会将被删除的元素作为返回值返回
- `unshift()`：向数组的开头添加一个或是多个元素
- `shift()`: 删除数组开头的元素
- 数组遍历
  - for循环`for(var i = 0; i < arr.length; i++){ 语句... }`
  - `forEach(function(value, index, 正在遍历的数组){ 语句...})`数组中有几个元素，函数就执行几次，回调函数会传入三个参数(value, index,数组本身)
- `slice(startIndex, [endIndex])`从数组返回选定的元素, 不回改变原数组, 包含开始不包含结束，`endIndex`输入负值则从后截取
- `splice(startIndex, 删除的数量，新元素1，新元素2)`, 从数组删除指定元素，改变原数组，返回被删除的元素
- `concat(arr1, arr2)`连接两个或是多个数组，并返回新的数组，不会改变原来的数组
- `join('连接符')`将数组转换为字符串，不会改变原数组，默认连接符为，
- `reverse()`颠倒数组，会直接改变数组
- `sort()`对数组进行排序，会影响原数组，会按照Unicode进行排序；
...
- 嵌套for循环
- `break`中止当前循环 -不能用在if语句中；`-label`：循环语句；`continue`跳过当次循环；二者只会对离它最近的语句起作用；用于性能提升
